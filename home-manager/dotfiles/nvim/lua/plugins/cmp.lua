...
local function _1_()
  ...
  cmp.setup({snippet = {expand = _2_}, window = {completion = cmp.config.window.bordered(), documentation = cmp.config.window.bordered()}, mapping = cmp.mapping.preset.insert({["<C-n>"] = cmp.mapping.select_next_item(), ["<C-p>"] = cmp.mapping.select_prev_item(), ["<C-e>"] = cmp.mapping.abort(), ["<CR>"] = cmp.mapping(_3_), ["<Tab>"] = cmp.mapping(_7_, {"i", "s"}), ["<S-Tab>"] = cmp.mapping(_9_, {"i", "s"})}), sources = cmp.config.sources({{name = "nvim_lsp"}, {name = "luasnip"}, {name = "path"}, {name = "codecompanion"}}, {{name = "buffer"}})})
